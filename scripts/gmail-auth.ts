...
if (!clientId || !clientSecret) {
  console.error("Missing GOOGLE_EMAIL_CLIENT_ID/SECRET or GOOGLE_CLIENT_ID/SECRET.");
  process.exit(1);
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    res.end("Rocky is connected to Gmail. You can close this tab.");
    console.log("\nCopy this into your .env:\n");
    console.log("GOOGLE_EMAIL_ENABLED=true");
    console.log(`GOOGLE_EMAIL_REFRESH_TOKEN=${json.refresh_token}`);
    shutdown(0);
  } catch (err) {
    res.statusCode = 500;
...
server.listen(port, "127.0.0.1", () => {
  console.log(`Open this URL to connect Rocky's Gmail account:\n\n${authUrl.toString()}\n`);
});
